...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> str | None:
        ...
        loop.run_until_complete(self.sync_plans()) 
    ...
    async def sync_plans(self):
        ...
        if all_variants:
            ...
            await self.save_variants(response)
    ...
    async def save_variants(self, data: list[dict]):
        ...
        """
        for info in data:
            self.stdout.write(f"Prepping to sync variant {info['name']} with the database...")
        ...
        for plan in saved_plans:
            self.stdout.write(f"{plan.name} synced with the database...")
